...
module.exports = async (ctx) => {
  ...
  // ── Configuration ──────────────────────────────────────────
  const API_URL = 'https://api.princetechn.com/api/anime/konachan?apikey=prince'
  const TEMP_DIR = path.join(__dirname, '..', 'temp')
  
  // Ensure your /temp directory exists 📂
  if (!fs.existsSync(TEMP_DIR)) fs.mkdirSync(TEMP_DIR, { recursive: true })

  await reply('⏳ *Fetching Konachan content...*')
  ...
  try {
    ...
    console.error('Konachan command error:', error)
