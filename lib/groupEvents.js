...
// ── Check bot admin from already-fetched groupMeta ────────────
// Uses the groupMeta we already have in hand.
// NO second groupMetadata() call — that was causing rejections.
const isBotAdminFromMeta = (sock, groupMeta) => {
  if (!groupMeta) return false
  const botNum = jidToNum(sock.user?.id || '')
  if (!botNum) return false
  return (groupMeta.participants || []).some(p => {
    const pNum = jidToNum(safeJid(p))
...
const handleGroupEvents = async (sock, update) => {
  ...
  try {
    ...
    // ── Admin gate — uses groupMeta already fetched above ─────
    // No second async call. No rejection risk.
    const botAdmin = isBotAdminFromMeta(sock, groupMeta)
    if (!botAdmin) {
    ...
    switch (action) {
      ...
      // ── Member promoted ───────────────────────────────────────
      case 'promote': {
        // Actor: who triggered this — from update.author (Baileys field)
        ...
        for (const p of participants) {
          ...
          // mentions: target always included, actor added if present
          const mentions = (actorRaw && actorRaw !== participant)
            ? [participant, actorRaw]
            : [participant]

          // actor line only shown when we have an actor
      ...
      case 'demote': {
        ...
        for (const p of participants) {
          ...
          if (num === botNum) {
            ...
            // Pause active counter when demoted
